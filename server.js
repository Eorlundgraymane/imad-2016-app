...
app.get('/userlist',function(req,res){//in case the db is called
	...
	pool.query('SELECT name FROM "codeusers";', function(err , result){//what query to be passed
		if(err){
			...
			var len = JSON.stringify(result.rows[0].fname).length;
			res.send('<html><link href="/forgestyle.css" rel="stylesheet"/><script type="text/javascript" src="/forgemain.js"></script><script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js"></script><body id = "container"><div id = "forgehammer">Welcome to the Highway '+JSON.stringify(result.rows[0].fname).substring(1,len-1)+'<br>Your name was indeed dynamically retreived from the database via an SQL query</div><br><br><div id = "leftmarg">More Updates Coming Soon</div></body></html>');//else return the query result as a JSON string
		}
	});
});
